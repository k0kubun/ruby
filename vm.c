...
{
    // Copying an environment to the heap reads the frame's locals straight out of the
    // VM stack, and vm_make_env_each() walks the EP chain into older frames doing the
    // same. A ZJIT frame does not keep its locals there unless something can reach them
    // through the EP chain, so ask every live ZJIT frame at or below this one to publish
    // them first. This is the one path by which Binding, eval and escaping Procs read a
    // JIT frame's locals without going through a materialization point.
...
#if USE_ZJIT
// Write back the Ruby stack slots (operand stack and locals) that a live ZJIT frame
// keeps in registers or on the native stack, as described by its JITFrame stack map.
//
// The map is a list of opcodes for a write cursor that starts at cfp->sp (or at the
// saved SP anchor named by a leading BASE_PTR entry) and moves down the VM stack.
// See the ZJIT_STACK_MAP_* macros in zjit.h.
static void
zjit_replay_stack_map(const rb_control_frame_t *cfp, const zjit_jit_frame_t *jit_frame)
{
...
// Publish the locals of every live ZJIT frame at or below `cfp` into the VM stack,
// without otherwise materializing the frames.
//
// ZJIT does not store a frame's locals to its environment on every call anymore: for an
// ISEQ that cannot hand its EP to a child frame, the locals live only in registers and
// native stack slots, and the JITFrame stack map says where. Anything that reads those
// locals out of the VM stack (vm_make_env_object() copying the environment to the heap
// for a Binding, an eval, or an escaping Proc) has to ask for them first.
//
// Unlike rb_zjit_materialize_frames(), this leaves cfp->pc, cfp->iseq and
// cfp->jit_return alone, so the JIT code these frames are executing keeps running
// afterwards. Replaying the map is idempotent: it writes the same values the frame
// would have stored eagerly.
void
rb_zjit_materialize_locals(const rb_execution_context_t *ec, const rb_control_frame_t *cfp)
{
    if (!rb_zjit_enabled_p) return;
    const rb_control_frame_t *end_cfp = ec->tag ? ec->tag->cfp : NULL;
    if (end_cfp == NULL || cfp > end_cfp) return;

    while (true) {
        if (CFP_ZJIT_FRAME_P(cfp)) {
            zjit_replay_stack_map(cfp, CFP_ZJIT_FRAME(cfp));
        }
...
zjit_materialize_frames(const rb_execution_context_t *ec, rb_control_frame_t *cfp, bool materialize_target)
{
    ...
    while (true) {
        ...
        if (CFP_ZJIT_FRAME_P(cfp)) {
            ...
            zjit_replay_stack_map(cfp, jit_frame);
            cfp->jit_return = 0;
        }
        if (end_cfp == cfp) break;
        cfp = RUBY_VM_PREVIOUS_CONTROL_FRAME(cfp);
