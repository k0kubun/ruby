...
fn can_direct_send_forwardable(caller_args: &CallerArguments) -> Result<(), SendDirectFailure> {
    ...
    if caller_args.ci.is_none() || caller_args.flags & FORWARDABLE_CALLEE_BLOCKERS != 0 {
...
impl Function {
    ...
    fn specialize_send_forward(&mut self, block: BlockId, insn_id: InsnId) -> bool {
        ...
        let Some(frame_has_block) = candidates.iter().copied().find(|&has_block|
            self.build_send_direct_args(&CallerArguments::new(&probe, shape), None, iseq, has_block, false).is_ok()
        ) else {
            incr_counter!(send_forward_reject_complex_args);
            self.set_dynamic_send_reason(insn_id, SendForwardComplexArgs);
            return false;
        ...
        let merged_args = match forwarding {
            ...
            ForwardedCallInfo::Profiled { ci, local_table_size } => {
                ...
                for i in 0..caller_argc {
                    merged_args.push(self.load_field(block, ep, FieldName::StackSlot,
        ...
        let Ok(call) = self.build_send_direct_args(&merged_caller_args, None, iseq, frame_has_block, false) else {
                ...
                match resolved.insn(self) {
                    ...
                    &Insn::Send { mut recv, cd, state, block: send_block, caller_splat_length, .. } => {
                        ...
                        if def_type == VM_METHOD_TYPE_ISEQ {
                            ...
                            // The site's `ci` counts the arguments the *bytecode* pushed, which is
                            // one more than the call has once a `send` name or a Symbol block's
                            // receiver has been taken off the front.
                            let ccall_argc = if send_mid_override.is_some() || symbol_block_mid.is_some() {
                                args.len() as u32
                            } else {
                                unsafe { vm_ci_argc(ci) }
                            };
    ...
    fn reduce_block_params(&mut self) {
        ...
        for block in self.reverse_post_order() {
            ...
            let params = self.blocks[block].params.clone();
            let mut consts = vec![];
            for (idx, &param) in params.iter().enumerate() {
                let Some(val) = self.type_of(param).exact_ruby_value() else {
                    continue
                };
                dropped[block].push(idx);
            ...
            self.blocks[block].insns.splice(0..0, prologue);
            // Keep the surviving parameters in order.
            let drop_set: HashSet<usize> = dropped[block].iter().copied().collect();
            self.blocks[block].params = params.into_iter().enumerate()
        ...
        let retain_args = |edge: &mut BranchEdge, dropped: &[Vec<usize>]| {
            let drop_set = &dropped[edge.target];
        ...
        for block in self.reverse_post_order() {
            ...
            match &mut self.insns[terminator_id] {
        ...
        while insn_idx < iseq_size {
            ...
            match opcode {
                ...
                YARVINSN_getblockparamproxy => {
                    ...
                    // Inline every block handler case the interpreter handles instead of
                    // specializing on a profile: iseq/ifunc -> nil -> proc, and side-exit
                    // on the remaining symbol handler, which needs a memoized Proc in EP.
                    ...
                    let jump_to_join = |fun: &mut Function, from: BlockId, val: InsnId| {
                    ...
                    jump_to_join(fun, iseq_or_ifunc_block, proxy_val);

                    // No block was passed: the block param is nil.
                    ...
                    jump_to_join(fun, nil_block, nil_val);

                    // A Proc block handler is the Proc itself.
                    ...
                    jump_to_join(fun, proc_block, proc_val);
                ...
                YARVINSN_invokeblock => {
                    ...
                    //
                    // Skipped once the lone yielded Array was auto-splatted: `args` no longer
                    // holds what the `yield` passed, and a Symbol block sends to the first of
                    // those arguments.
                    let mut symbol_handlers: Vec<VALUE> = vec![];
                    if can_direct_invoke_block(flags) && !args.is_empty() && autosplat_join.is_none() {
                        if let Some(summary) = block_handler_summary.as_ref() {
                            for &profiled_type in summary.buckets() {
                                if profiled_type.is_empty() {
                                    break;
