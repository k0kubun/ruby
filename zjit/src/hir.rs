                ...
                match resolved.insn(self) {
                    ...
                    &Insn::Send { mut recv, cd, state, block: send_block, caller_splat_length, .. } => {
                        ...
                        if def_type == VM_METHOD_TYPE_ISEQ {
                            ...
                            if reduce_send_to_ccall(self, block, insn_id, recv, cd, send_block, args, state, klass, profiled_type, cme, mid, unsafe { vm_ci_argc(ci) }, flags_for_check, send_block_arg).is_ok() {
                                continue;
                            }
