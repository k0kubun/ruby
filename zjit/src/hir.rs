                    ...
                    &Insn::Send { mut recv, cd, state, block: send_block, caller_splat_length, .. } => {
                        ...
                        if def_type == VM_METHOD_TYPE_ISEQ {
                            ...
                            fn reduce_send_to_ccall(
                                ...
                                // The method the call resolves to and the argument count it
                                // really has. Both normally come from `cd`'s call info, but a
                                // `yield` to a Symbol block borrows the yield site's `cd`: its
                                // `ci` names no method and counts the Symbol block's receiver
                                // among the arguments.
                                method_id: ID,
                                argc: u32,
                            ) -> Result<(), ()> {
                ...
                if get_option!(stats) {
                    ...
                    match payload.profile.get_block_handlers(iseq_insn_idx) {
                        ...
                        Some(summary) if summary.is_skewed_polymorphic() || summary.is_polymorphic() => {
                            if summary.buckets().iter().any(|ty| !ty.is_empty() && unsafe { rb_IMEMO_TYPE_P(ty.class(), imemo_ifunc) == 1 }) {
