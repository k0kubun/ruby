...
impl Function {
    ...
    fn specialize_send_forward(&mut self, block: BlockId, insn_id: InsnId) -> bool {
        ...
        let Some(frame_has_block) = candidates.iter().copied().find(|&has_block|
            self.build_send_direct_args(&CallerArguments::new(&probe, shape), None, iseq, has_block).is_ok()
        ) else {
            incr_counter!(send_forward_reject_complex_args);
            self.set_dynamic_send_reason(insn_id, SendForwardComplexArgs);
            return false;
        ...
        let Ok(call) = self.build_send_direct_args(&merged_caller_args, None, iseq, frame_has_block) else {
        ...
        while insn_idx < iseq_size {
            ...
            match opcode {
                ...
                YARVINSN_leave => {
                    ...
                    let val = state.stack_pop()?;
                    match mode {
                        AddIseqMode::Standalone | AddIseqMode::ExceptionEntry(_) => fun.push_insn(block, Insn::Return { val }),
