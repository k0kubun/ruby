...
macro_rules! for_each_operand_impl {
    ($self:expr, $visit_one:ident, $visit_many:ident) => {
        match $self {
            ...
            Insn::PushInlineFrame { recv, block_arg, state, .. } => {
                $visit_one!(*recv);
...
impl<'a> std::fmt::Display for InsnPrinter<'a> {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        ...
        match &self.inner {
            ...
            Insn::PushInlineFrame { recv, iseq, cme, num_args, block_arg, .. } => {
                let method_name = unsafe { (**cme).called_id };
        ...
        while let Some(block) = worklist.pop_front() {
            ...
            loop {
                ...
                self.push_insn(block, Insn::PushInlineFrame {
                    iseq, cme, recv, num_args: args.len().try_into().unwrap(), blockiseq, block_arg, state,
                });
                self.count(block, Counter::inline_iseq_optimized_send_count);
                ...
                YARVINSN_send => {
                    ...
                    let proxy_join = if proxy_split {
                        ...
                        *proxy_args.last_mut().unwrap() = fun.push_insn(proxy_block, Insn::RefineType { val: block_arg_insn, new_type: Type::from_value(proxy) });
                        let proxy_send = fun.push_insn(proxy_block, Insn::Send { recv, cd, block: block_handler, args: proxy_args, caller_splat_length, state: exit_id, reason: Uncategorized(opcode.into()) });
                    ...
                    if split_nil_block_arg {
                        ...
                        *nil_args.last_mut().unwrap() = fun.push_insn(nil_block, Insn::RefineType { val: block_arg_insn, new_type: types::NilClass });
                        let nil_send = fun.push_insn(nil_block, Insn::Send { recv, cd, block: block_handler, args: nil_args, caller_splat_length, state: exit_id, reason: Uncategorized(opcode.into()) });
