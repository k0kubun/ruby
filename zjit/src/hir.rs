...
macro_rules! for_each_operand_impl {
    ($self:expr, $visit_one:ident, $visit_many:ident) => {
        match $self {
            ...
            Insn::PushInlineFrame { recv, block_arg, captured, state, guard_state, .. } => {
                $visit_one!(*recv);
        ...
        while let Some(block) = worklist.pop_front() {
            ...
            loop {
                ...
                self.push_insn(block, Insn::PushInlineFrame {
                    iseq, cme, recv, num_args: args.len().try_into().unwrap(), blockiseq, block_arg, captured: None, state, guard_state,
                });
