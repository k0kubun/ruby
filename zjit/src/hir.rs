...
pub enum Insn {
    ...
    /// send site is: probe the class table for the receiver's class, and serve the target
    /// in JIT code when it is one of the kinds [`crate::codegen::gen_send_megamorphic_direct`]
    /// serves -- an `attr_reader` or a C method as much as an ISEQ method, which matters here
    /// because `&:name` over a heterogeneous collection is mostly the first two.
    ///
...
macro_rules! for_each_operand_impl {
    ($self:expr, $visit_one:ident, $visit_many:ident) => {
        match $self {
            ...
            Insn::PushInlineFrame { recv, block_arg, captured, forwarded, state, guard_state, .. } => {
                $visit_one!(*recv);
...
impl Function {
    ...
    fn push_symbol_block_dispatch(
        ...
        for (idx, &symbol) in symbols.iter().enumerate() {
            ...
            let send = self.push_insn(send_block, Insn::Send {
                recv: args[0], cd, block: None, args: args[1..].to_vec(), state,
    ...
    fn specialize_send_forward(&mut self, block: BlockId, insn_id: InsnId) -> bool {
        ...
        let Some(frame_has_block) = candidates.iter().copied().find(|&has_block|
            self.build_send_direct_args(&probe, shape, iseq, has_block, has_block, false).is_ok()
        ) else {
            incr_counter!(send_forward_reject_complex_args);
            self.set_dynamic_send_reason(insn_id, SendForwardComplexArgs);
            return false;
        ...
        let Ok(call) = self.build_send_direct_args(&merged_args, shape, iseq, frame_has_block, frame_has_block, false) else {
        ...
        let replacement = self.push_insn(block, Insn::SendDirect(Box::new(SendDirectData {
            ...
            state: send_state, guard_state: state, block: None, block_arg,
        })));
        self.make_equal_to(insn_id, replacement);
    ...
    fn inline_methods(&mut self) -> bool {
        ...
        while let Some(block) = worklist.pop_front() {
            ...
            loop {
                ...
                let SendDirectData { recv, cd, cme, iseq, kw_bits, jit_entry_idx, block: call_block, block_arg, state, guard_state, .. } = **data;
                ...
                self.push_insn(block, Insn::PushInlineFrame {
                    iseq, cme, recv, num_args: args.len().try_into().unwrap(), blockiseq, block_arg, captured: None, forwarded, state, guard_state,
                });
    ...
    fn gen_post_send_no_ep_escape_patch_point(&mut self, block: BlockId, state: &FrameState, insn_idx: u32) {
        ...
        let reload_exit_id = self.push_insn(block, Insn::Snapshot { state: Box::new(reload_state.without_locals_if_eager()) });
    ...
    fn push_symbol_block_mega(&mut self, block: BlockId, insn_id: InsnId) -> bool {
        ...
        // argument, which `shape_servable` rechecks below.
        let flags = unsafe { rb_vm_ci_flag((*cd).ci) };
        // Nor `VM_CALL_FCALL`, which is load-bearing rather than incidental: the flags become
        // the table's, and `zjit_send_cache_direct_cme()` reads `VM_CALL_FCALL` off the table
        // as permission to serve a *private* method inline. `vm_call_symbol()` sends a private
        // method to `method_missing` instead, so a `yield` that somehow carried the flag would
        // have to stay off the table rather than be dispatched through it.
...
impl FrameState {
    ...
    /// Like [`Self::without_locals`], but only for a frame that keeps its locals stored
    /// in its environment at every call. Dropping the locals from a side exit is only
    /// safe when something else already put them where the interpreter looks; a frame
    /// whose locals live in JITFrame stack maps has to write them on the exit path.
    /// See `codegen::iseq_needs_eager_locals`.
    fn without_locals_if_eager(&self) -> Self {
        if crate::codegen::iseq_needs_eager_locals(self.iseq) {
            self.without_locals()
        } else {
            self.clone()
        }
            ...
            match opcode {
                ...
                YARVINSN_checkkeyword => {
                    ...
                    let val = if !local_inval {
                        ...
                        let exit_id = fun.push_insn(block, Insn::Snapshot { state: Box::new(exit_state.without_locals_if_eager()) });
                ...
                YARVINSN_getlocal_WC_0 => {
                    ...
                    if !local_inval {
                        ...
                        let exit_id = fun.push_insn(block, Insn::Snapshot { state: Box::new(exit_state.without_locals_if_eager()) }); // skip spilling locals
                        fun.push_insn(block, Insn::PatchPoint { invariant: Invariant::NoEPEscape(iseq), state: exit_id });
                        local_inval = false;
