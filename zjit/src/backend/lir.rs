...
/// at frame push), an already-guarded Proc VALUE, or a block handler already loaded
/// from the caller's frame (the block param proxy case).
...
impl Interval {
    ...
    /// Check if the interval is alive strictly inside one of its ranges, i.e. it was
    /// live before `position` and is still live after it. Unlike [`Self::covers`] this
    /// excludes an interval that merely starts at `position`.
        ...
        for block_id in self.block_order() {
            ...
            for (insn, insn_id) in old_insns.into_iter().zip(old_ids.into_iter()) {
                if let Insn::CCall { data } = insn {
                    ...
                    // Only an output whose allocation really begins at this call can be
                    // left out of the survivors below. A coalesced output shares its
                    // allocation with the rest of its web, which may be holding a value
                    // that is live across the call, so it has to be saved like any other.
                    let out_vreg_id = out.is_vreg()
                        .then(|| intervals[out.vreg_idx()].owner(intervals).vreg_id)
                        .filter(|&id| !intervals[id].has_bounds() || intervals[id].born_at(insn_number));
                    let survivors: Vec<VRegId> = intervals.iter()
                        .filter(|interval| {
                            ...
                            // doesn't apply to it. Case 2 still does: the stack map can only
                            // point at a VM stack slot in memory, so the value has to be
                            // written somewhere the materializer can read it.
                            let needs_save = if pool_index.is_some_and(|index| alloc_regs.is_callee_saved(index)) {
                                stack_map_reg
                            } else {
                                live_across_call || stack_map_reg
                            };
    ...
    pub fn compile_exits(&mut self, exit_islands: bool) -> Vec<Insn> {
        ...
        fn compile_exit_meta_index(
            ...
            let meta = ExitMeta {
                ...
                iseq: exit.iseq,
                compiled_version: match recompile {
                    Some(SideExitRecompile { compiled_version: Opnd::UImm(version) }) =>
                        *version as *mut crate::payload::IseqVersion,
                    Some(recompile) => unreachable!(
                        "recompile target should be a constant version pointer, got {:?}", recompile.compiled_version
                    ),
