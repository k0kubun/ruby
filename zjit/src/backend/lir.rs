...
impl Assembler
{
    ...
    pub fn compile_exits(&mut self) -> Vec<Insn> {
        ...
        ///
        /// `target` is `materialize_exit_trampoline` for a traced exit, which has
        /// already restored the frame's interpreter state inline, and
        /// `exit_meta_trampoline` for an ordinary one, which reads that state out of
        /// the [`crate::exit_meta`] table instead.
            ...
            if let Target::SideExit(data) = target {
                ...
                let compiled_exit = match compiled_exits.entry(exit) {
                    ...
                    Entry::Vacant(entry) => {
                        ...
                        // Nothing to restore before the jump on this branch -- the
                        // allocator hands out no callee-saved registers here -- so exits
                        // jump straight to the trampoline instead of through a shared tail.
                        let tail = Target::CodePtr(ZJITState::get_exit_meta_trampoline());
