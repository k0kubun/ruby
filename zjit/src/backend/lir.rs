...
impl Assembler
{
    ...
    pub fn compile(self, cb: &mut CodeBlock) -> Result<(CodePtr, Vec<CodePtr>), CompileError> {
        ...
        // `Insn::BeginOutlined` leaves emission pointed at the outlined half of the
        // region. Everything after a compile -- the next compile, a patch, the caller
        // reading `cb.get_write_ptr()` for the end of the function body -- expects
        // the half we came in on, so put it back however the compile turned out.
