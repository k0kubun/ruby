...
use crate::cruby::{IseqPtr, RUBY_OFFSET_CFP_ISEQ, RUBY_OFFSET_CFP_JIT_RETURN, RUBY_OFFSET_CFP_PC, RUBY_OFFSET_CFP_SP, SIZEOF_VALUE_I32, VALUE, ZJIT_STACK_MAP_SHIFT, ZJIT_STACK_MAP_VREG_TAG, vm_stack_canary, zjit_jit_frame};
                        ...
                        for (idx, stack_opnd) in stack.iter().enumerate() {
                            let entry = match stack_opnd {
                                ...
                                Opnd::VReg { idx: VRegId(vreg_id), .. } => {
                                    ...
                                    let encoded = (stack_index << ZJIT_STACK_MAP_SHIFT) | ZJIT_STACK_MAP_VREG_TAG as usize;
