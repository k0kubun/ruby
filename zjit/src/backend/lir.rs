...
impl Assembler {
    ...
    /// Wrap an instruction in a HIR perf symbol range when HIR perf output is enabled.
    pub(super) fn push_insn_with_perf_symbol(&mut self, symbol_name: &str, insn: Insn) {
        if let Some((start, end)) = perf_symbol_markers(symbol_name, false) {
            self.push_insn(start);
            self.push_insn(insn);
            self.push_insn(end);
        } else {
            self.push_insn(insn);
