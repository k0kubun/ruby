...
impl Invariants {
    ...
    pub fn heap_size(&self) -> (usize, usize) {
        ...
        account_map!(self.no_method_override_patch_points, ID);
        account_map!(self.constant_state_patch_points, ID);
        account_map!(self.no_singleton_class_patch_points, VALUE);
        // Not patch points, so it adds bytes but no count: a memo table that is
        // dropped whole whenever an answer could change.
        bytes += hash_table_bytes::<((VALUE, ID), bool)>(self.no_override_cache.capacity());
