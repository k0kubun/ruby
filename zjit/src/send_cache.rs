...
pub fn cache_for(cd: *const rb_call_data, reason: crate::hir::SendFallbackReason) -> Option<*const SendCache> {
    ...
    // `SendPolymorphicFallback` is the fallthrough arm of a receiver guard chain, which
    // is reached by exactly the classes the chain could not cover.
    match reason {
        SendMegamorphic | SendAncestorGuardFallback | SendPolymorphic | SendPolymorphicFallback => {}
        _ => return None,
    }
