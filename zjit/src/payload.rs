...
impl IseqPayload {
    ...
    /// Maximum number of versions this ISEQ may compile. Starts at `--zjit-max-versions`
    /// and grows by one for each version killed by PatchPoint invalidation, so that
    /// invalidated (dead) code does not eat into the budget meant for respecialization.
    pub fn version_limit(&self) -> usize {
        crate::codegen::max_iseq_versions() + self.invalidation_recompiles as usize
    }
