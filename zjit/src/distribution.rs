...
impl<T: Copy + PartialEq + Default, const N: usize> Distribution<T, N> {
    ...
    pub fn observe_stable(&mut self, item: T, matches: impl Fn(T, T) -> bool) -> StableBucket {
        ...
        // A second distinct item: materialize buckets 1..N, like observe() does.
        let tail = self.tail.get_or_insert_with(|| Box::new(Tail::new()));
        for idx in 1..N {
            if tail.counts[idx] == 0 {
                tail.buckets[idx] = item;
                tail.counts[idx] = 1;
                return StableBucket::Inserted(idx);
            }
            if matches(tail.buckets[idx], item) {
                return StableBucket::Existing(idx);
            }
        }
        self.other = self.other.saturating_add(1);
        StableBucket::Full
    ...
    ///
    /// Dropping the tail hands its allocation back, so a site that evicts also stops paying for
    /// the buckets it just gave up.
