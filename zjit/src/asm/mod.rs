...
impl CodeBlock {
    ...
    /// Direct the next writes at the outlined (cold) half of the region, or back at the inlined half.
    ...
    /// The last offset that the current half accepts writes at.
...
impl CodeBlock {
    ...
    /// Stub CodeBlock whose region is split into an inlined and an outlined half.
...
mod tests
{
    ...
    fn test_set_outlined_keeps_a_write_position_per_half() {
        ...
        // Switching halves remembers the inlined write position rather than losing it.
    ...
    fn test_unsplit_code_block_ignores_set_outlined() {
        // CodeBlock without split (used for JITFrame region) should not have an outlined half.
