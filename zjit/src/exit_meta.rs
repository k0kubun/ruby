...
impl ExitMeta {
    /// Keep the ISEQ pointers alive, like `JITFrame` does. A record is never freed,
    /// so the exit it belongs to can run at any point in the future and has to be
    /// able to resume into (or recompile) these ISEQs.
    pub fn mark(&self) {
...
pub fn intern(meta: ExitMeta) -> u32 {
    ...
    // forever; `rb_zjit_root_mark` walks the table and marks them.
