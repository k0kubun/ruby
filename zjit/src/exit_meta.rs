...
impl ExitMeta {
    /// Mark the ISEQ pointers for GC, like `JITFrame` does. Called from
    /// rb_zjit_root_mark: an exit that has not run yet still has to be able to
    /// resume into (or recompile) its ISEQ.
    pub fn mark(&self) {
...
pub fn intern(meta: ExitMeta) -> u32 {
    // The record's ISEQs have to stay alive for as long as the record does, which
    // is forever; rb_zjit_root_mark walks this table to keep them alive.
