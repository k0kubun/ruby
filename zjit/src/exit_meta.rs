...
//! with the compiled version it is charging the exit to. All of that is known when the
...
pub struct ExitMeta {
    ...
    pub iseq: IseqPtr,
    /// The compiled version to invalidate to force a recompile. For an exit out of
    /// inlined code this belongs to the outer ISEQ the callee was folded into, so it
    /// is not necessarily a version of `iseq`. It is a Rust-heap pointer, so unlike
    /// `iseq` it needs no compaction fixup. Null when this exit does not
    /// count toward recompilation.
    pub compiled_version: *mut IseqVersion,
...
impl ExitMeta {
    /// Update the ISEQ pointer after GC compaction, like `JITFrame` does. `pc`
    /// points into the ISEQ's malloc'd `iseq_encoded`, which compaction does not
    /// move, and `compiled_version` lives on the Rust heap, so neither needs a fixup.
...
pub fn intern(meta: ExitMeta) -> u32 {
    // The record's ISEQ has to stay alive for as long as the record does, which is
