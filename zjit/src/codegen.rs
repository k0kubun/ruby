...
use crate::payload::{ExceptionEntry, ExceptionEntrySpec, IseqCodePtrs, IseqStatus, IseqVersion, IseqVersionRef, JITFrame, MAX_INVALIDATION_RECOMPILES, get_or_create_iseq_payload};
...
use crate::hir::{BlockHandler, callee_passes_kw_bits_arg, CCallVariadicData, CCallWithFrameData, CondBranchFixnumCmpData, CondBranchHasTypeData, Const, FieldName, FixnumCmpOp, FrameState, Function, Insn, InsnId, Recompile, SendDirectData, SendFallbackReason, qualified_method_name};
...
pub fn invalidate_iseq_version(cb: &mut CodeBlock, iseq: IseqPtr, version: &mut IseqVersionRef) {
    ...
    // paid for out of the respecialization budget. This is separate from the
    // --zjit-num-exits-until-invalidate countdown, which budgets how many recompile side
    // exits a version tolerates before exit_recompile invalidates it: that countdown decides
    // *when* mis-speculated code is thrown away, while this grant decides whether an
    // externally invalidated ISEQ may compile a replacement at all.
    if payload.version_count_for(*version) >= payload.version_limit() {
...
pub fn gen_exception_entry_trampoline(cb: &mut CodeBlock) -> Result<CodePtr, CompileError> {
    ...
    let (code_ptr, gc_offsets) = asm.compile(cb)?;
    assert!(gc_offsets.is_empty());
    perf::register_current_code_range(cb, "exception entry trampoline", code_ptr);
    Ok(code_ptr)
}
...
fn gen_insn(cb: &mut CodeBlock, jit: &mut JITState, asm: &mut Assembler, function: &Function, insn_id: InsnId, insn: &Insn) -> Result<(), InsnId> {
    ...
    let out_opnd = match insn {
        ...
        Insn::CCallWithFrame(insn) => {
            ...
            // A reduced `&blk` argument's handler goes into the C frame's specval as is.
            let block_arg = match *block {
                Some(BlockHandler::BlockArgProc(block_arg) | BlockHandler::BlockParamProxy(block_arg)) => Some(opnd!(block_arg)),
                _ => None,
            };
...
fn gen_getivar(jit: &mut JITState, asm: &mut Assembler, function: &Function, recv_id: InsnId, recv: Opnd, id: ID, ic: *const iseq_inline_iv_cache_entry, state: &FrameState) -> Opnd {
    let recv_type = function.type_of(recv_id);
    if get_option!(disable_ivar_cache) {
        return gen_getivar_generic(jit, asm, function, recv_type, recv, id, ic, state);
    }
    ...
    if recv_type.could_be(types::Module) {
...
fn gen_getivar_generic(jit: &mut JITState, asm: &mut Assembler, function: &Function, recv_type: Type, recv: Opnd, id: ID, ic: *const iseq_inline_iv_cache_entry, state: &FrameState) -> Opnd {
    gen_trace_fallback(asm, "getivar");
    if recv_type.could_be(types::Module) {
        // Reading a class or module ivar from a non-owner Ractor raises Ractor::IsolationError.
        gen_prepare_non_leaf_call(jit, asm, function, state);
...
c_callable! {
    ...
    pub(crate) fn exit_recompile(compiled_version_raw: *mut IseqVersion) {
        ...
        // Fast check before taking the VM lock: skip if the compiled version is already
        ...
        // The check is on the compiled version because that is the version we invalidate.
        {
            let payload = get_or_create_iseq_payload(compiled_iseq);
            let version_count = payload.version_count_for(compiled_version);
            if unsafe { compiled_version.as_ref() }.is_invalidated() || version_count >= payload.version_limit() {
                return;
            }
        ...
        with_vm_lock(src_loc!(), || {
            ...
            if !unsafe { compiled_version.as_ref() }.is_invalidated() {
                let cb = ZJITState::get_code_block();
...
pub fn gen_exception_entry_stub_hit_trampoline(
    ...
    asm.compile(cb).map(|(code_ptr, gc_offsets)| {
        assert!(gc_offsets.is_empty());
        perf::register_current_code_range(cb, "exception entry stub hit trampoline", code_ptr);
        code_ptr
    })
}
...
pub fn gen_exception_entry_stub_exit(cb: &mut CodeBlock) -> Result<CodePtr, CompileError> {
    ...
    asm.compile(cb).map(|(code_ptr, gc_offsets)| {
        assert!(gc_offsets.is_empty());
        perf::register_current_code_range(cb, "exception entry stub exit", code_ptr);
        code_ptr
    })
}
...
pub fn gen_jit_entry_call_trampoline(cb: &mut CodeBlock) -> Result<CodePtr, CompileError> {
    ...
    asm.compile(cb).map(|(code_ptr, gc_offsets)| {
        assert_eq!(gc_offsets.len(), 0);
        perf::register_current_code_range(cb, "jit entry call trampoline", code_ptr);
        code_ptr
    })
}
...
c_callable! {
    ...
    pub(crate) fn rb_zjit_side_exit_from_meta(ec: EcPtr, cfp: CfpPtr, sp: *mut VALUE, meta_idx: u32) {
        ...
        if !meta.compiled_version.is_null() {
            exit_recompile(meta.compiled_version);
        }
...
pub fn gen_exit_meta_trampoline(cb: &mut CodeBlock, exit_trampoline: CodePtr) -> Result<CodePtr, CompileError> {
    ...
    asm.compile(cb).map(|(code_ptr, gc_offsets)| {
        assert_eq!(gc_offsets.len(), 0);
        perf::register_current_code_range(cb, "exit_meta trampoline", code_ptr);
        code_ptr
    })
}
