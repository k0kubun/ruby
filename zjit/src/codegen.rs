...
struct JITState {
    ...
    /// Stack size reserved for spilled block params and JITFrame.
    /// See also: Assembler::param_opnd() and gen_save_pc_for_gc()
    reserved_stack_size: usize,
}

impl JITState {
    /// Create a new JITState instance
    fn new(version: IseqVersionRef, num_insns: usize, num_blocks: usize, reserved_stack_size: usize) -> Self {
        JITState {
            version,
...
fn gen_function(cb: &mut CodeBlock, iseq: IseqPtr, version: IseqVersionRef, function: &Function) -> Result<(IseqCodePtrs, Vec<CodePtr>, Vec<IseqCallRef>), CompileError> {
    let (mut jit, asm) = trace_compile_phase("codegen", || {
        // Reserve `num_spilled_params + 1` stack slots for spilled block params and JITFrame.
        // See also: Assembler::param_opnd() and gen_save_pc_for_gc()
        let num_spilled_params = max_num_params(function).saturating_sub(ALLOC_REGS.len());
        let reserved_stack_size = num_spilled_params + 1; // +1 for JITFrame
        let mut jit = JITState::new(version, function.num_insns(), function.num_blocks(), reserved_stack_size);
        let mut asm = Assembler::new();
        asm.alloc_stack(reserved_stack_size);
...
/// Reserve a native stack space and write operands into it
fn gen_push_opnds(jit: &JITState, asm: &mut Assembler, opnds: &[Opnd]) -> lir::Opnd {
    let argv = if opnds.len() > 0 {
        ...
        let total_stack_size = jit.reserved_stack_size + opnds.len();
        asm.alloc_stack(total_stack_size);
        asm.sub(NATIVE_BASE_PTR, (SIZEOF_VALUE * total_stack_size).into())
    } else {
