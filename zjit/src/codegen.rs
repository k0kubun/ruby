...
fn gen_insn(cb: &mut CodeBlock, jit: &mut JITState, asm: &mut Assembler, function: &Function, insn_id: InsnId, insn: &Insn) -> Result<(), InsnId> {
    ...
    let out_opnd = match insn {
        ...
        Insn::PushInlineFrame { cme, iseq, recv, num_args, blockiseq, block_arg, state } => {
            let block_arg = block_arg.map(|block_arg| opnd!(block_arg));
            no_output!(gen_push_inline_frame(jit, asm, function, *cme, *iseq, opnd!(recv), *num_args, &function.frame_state(*state), *blockiseq, block_arg))
        },
