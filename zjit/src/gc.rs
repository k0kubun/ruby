...
use crate::payload::{IseqPayload, IseqVersionRef};
...
fn iseq_update_references(payload: &mut IseqPayload) {
    ...
    for version in payload.versions.iter_mut() {
        ...
        for iseq_call in version.iseq_calls.borrow().iter() {
...
pub fn append_gc_offsets(iseq: IseqPtr, version: &IseqVersionRef, offsets: &Vec<CodePtr>) {
    version.gc_offsets.borrow_mut().extend(offsets);
