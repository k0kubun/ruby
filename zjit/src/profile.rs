...
/// it. `Distribution` boxes everything past bucket 0, so the width costs nothing until a site
/// goes polymorphic -- at which point there is no reason to size the box for six buckets whose
/// contents would never be read.
...
pub struct IseqProfile {
    ...
    /// Callinfos observed in the `...` local at `sendforward` sites. Boxed for the same reason
    /// as `super_cme`. Like `splat_lengths` and unlike the tables above, it holds no objects.
    ...
    /// reaching `rb_vm_invokeblock()`. Boxed and optional for the same reason as
    /// `block_handlers`, and additionally only allocated for a site that actually falls back.
...
impl IseqProfile {
    ...
    /// Mutable access to the forwarded callinfo distributions, creating the table on first
    /// use. Needs no marking invalidation for the same reason `splat_lengths_mut` does not: a
    /// packed callinfo is an immediate, not an object, and a heap one is recorded as `None`.
    ...
    fn observe_block_fallback(&mut self, iseq: IseqPtr, insn_idx: YarvInsnIdx, handler: VALUE, argc: usize, arg0: VALUE) -> BlockReprofiled {
        // The window distributions hold ISEQ imemos, Symbols and receiver classes, all of which
        // GC marking has to see.
        self.marked_objects_stale = true;
        let entry = self.block_fallbacks.get_or_insert_with(Default::default)
        ...
        let handlers = self.block_handlers.get_or_insert_with(Default::default)
    ...
    pub fn heap_size(&self) -> ProfileHeapSize {
        ...
        if let Some(block_handlers) = self.block_handlers.as_ref() {
            out.bytes += size_of::<HashMap<YarvInsnIdx, TypeDistribution>>();
            out.bytes += hash_table_bytes::<(YarvInsnIdx, TypeDistribution)>(block_handlers.capacity());
            out.bytes += block_handlers.values().map(TypeDistribution::heap_size).sum::<usize>();
        }
        if let Some(block_fallbacks) = self.block_fallbacks.as_ref() {
            out.bytes += size_of::<HashMap<YarvInsnIdx, BlockFallbackEntry>>();
            out.bytes += hash_table_bytes::<(YarvInsnIdx, BlockFallbackEntry)>(block_fallbacks.capacity());
            out.bytes += block_fallbacks.values()
                .map(|e| e.dist.heap_size() + e.symbol_recv.heap_size()).sum::<usize>();
        }
        ...
        if let Some(forwarded_cis) = self.forwarded_cis.as_ref() {
            out.bytes += size_of::<HashMap<YarvInsnIdx, ForwardedCiDistribution>>();
            out.bytes += hash_table_bytes::<(YarvInsnIdx, ForwardedCiDistribution)>(forwarded_cis.capacity());
            out.bytes += forwarded_cis.values().map(ForwardedCiDistribution::heap_size).sum::<usize>();
        }
        out
    }
