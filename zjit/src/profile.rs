...
pub struct IseqProfile {
    ...
    /// `send_mid_mut`, `block_handlers_mut`, `observe_block_fallback` and
    /// `each_object_mut`. Nothing else in this file may touch `entries`, `super_cme`,
    /// `send_mid`, `block_handlers` or `block_fallbacks`
    /// mutably without going through one of those -- a mutation that failed to set
    /// this flag would leave marking blind to a newly recorded object, i.e. a
    /// use-after-free.
...
impl IseqProfile {
    ...
    fn observe_block_fallback(&mut self, iseq: IseqPtr, insn_idx: YarvInsnIdx, handler: VALUE, argc: usize, arg0: VALUE) -> BlockReprofiled {
        // This records objects into `block_fallbacks` and folds them into `block_handlers`
        // directly, so the dense marking copy no longer describes the distributions.
        self.marked_objects_stale = true;
