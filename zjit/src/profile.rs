...
pub struct IseqProfile {
    ...
    /// can hand out a mutable distribution sets it: `entry_mut`, `super_cme_mut` and
    /// `each_object_mut`. Nothing else in this file may touch `entries` or `super_cme`
...
impl IseqProfile {
    ...
    /// Mutable access to the splat length distributions. Unlike the one above this
